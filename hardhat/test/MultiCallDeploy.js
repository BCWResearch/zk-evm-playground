...
describe('Wallet Factory', function () {
  ...
  describe('deployAndExecute', function () {
    it('Should deploy and execute transfers', async function () {
      ...
      // Signing
      const data = encodeMetaTransactionsData(cfa, [transaction, transaction], networkId, ethers.constants.Zero)
      const sig = walletMultiSign([{ weight: 1, owner: owner_a }], 1, data, false)

      // Execution
      await multiCall.connect(executor).deployAndExecute(cfa, mainModule.address, salt, factory.address, [transaction, transaction], 0, sig)
